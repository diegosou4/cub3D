...
void	draw_aux1(t_game *game)
{
	...
	{
		game->draw.d = (game->draw.y) * 256 - HEIGHT * 128
			+ game->draw.spriteHeight * 128;
		game->draw.texY = ((game->draw.d * game->texture[15].texture.img_height)
				/ game->draw.spriteHeight) / 256;
...
{
	game->draw.invDet = 1.0 / (game->player.camera.PlaneX * game->player.dirY
			- game->player.dirX * game->player.camera.PlaneY);
	game->draw.transformX = game->draw.invDet * (game->player.dirY
			* game->draw.enemyX - game->player.dirX * game->draw.enemyY);
	game->draw.transformY = game->draw.invDet * (-game->player.camera.PlaneY
			* game->draw.enemyX + game->player.camera.PlaneX
			* game->draw.enemyY);
	game->draw.horizontal_offset = sin(game->frameCtd * game->draw.frequency)
		* game->draw.amplitude;
}

void	enemy_val_aux2(t_game *game)
{
	game->draw.spriteScreenX = (int)((WIDTH / 2) * (1 + game->draw.transformX
				/ game->draw.transformY)) + (int)game->draw.horizontal_offset;
	...
	game->draw.drawStartX = -game->draw.spriteWidth / 2
		+ game->draw.spriteScreenX;
...
void	draw_enemy(t_game *game)
{
	...
	while (++i < game->num_enemies)
	{
			...
			&& game->draw.transformY < game->player.ray.perpWallDist)
		{
			...
			{
				game->draw.texX = (int)((game->draw.stripe
							- (-game->draw.spriteWidth / 2
								+ game->draw.spriteScreenX))
						* game->texture[15].texture.img_width
						/ game->draw.spriteWidth);
